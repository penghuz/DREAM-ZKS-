function [x,p_x,Z,y] = dream_zks(N,T,din,dout,Obs,sd,range,alpha)
% Please check and change prior.m and pdf.m for your own problem

[delta,c,c_star,n_CR,p_g] = deal(1,0.05,1e-12,3,0.2);          % Default of algorithmic parameters
k = 10; p_s = 0.1; m0 = max(N,20*din); n_d = 3*delta;          % DREAM_ZS algorithmic variables
x = nan(T,din,N); p_x = nan(T,N); y = nan(T,dout,N);           % Preallocate chains, density and model responses
...
Z = prior(m0,din);                                             % Create initial archive
[Z(m0-N+1:m0,din+1),Y] = pdf(Z(m0-N+1:m0,1:din),Obs,sd);       % Compute log density and model responses of initial population
X = Z(m0-N+1:m0,1:din); p_X = Z(m0-N+1:m0,din+1); m = m0;      % Define X population and prepare Z
x(1,:,1:N) = reshape(X',1,din,N); p_x(1,1:N) = p_X';
y(1,:,1:N) = reshape(Y',1,dout,N);                             % Store initial states, density and model responses
X_ind_p = zeros(N,1); K = cell(N,2); N1 = 50; id = nan(N,1);     % Algorithmic variable
